...
export default function Categories() {
  ...
  async function fetch(pageNum = 1) {
    setLoading(true);
    try {
      const res = await getCategories(pageNum, limit, search || undefined);
      setItems(res.data);
      setMeta(res.meta);
      setPage(pageNum);
    } catch (err) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await updateCategoryImage(editingItem.id, imageUrl);
      setItems((prev) =>
        prev.map((c) => (c.id === editingItem.id ? { ...c, image: imageUrl } : c))
      );
      setModalOpen(false);
      setEditingItem(null);
    } catch (err) {
  ...
  async function handleDelete(cat: Category) {
    if (!confirm(`Are you sure you want to delete "${cat.titleEn} / ${cat.titleAm}"?`)) return;
    try {
  ...
  return (
    <>
      <PageMeta title="Category 1 | Dashboard" description="Product categories" />
                  ...
                  <TableRow key={cat.id}>
                    ...
                    <TableCell className="px-5 py-4 sm:px-6 text-start">
                      <span className="font-medium text-gray-800 text-theme-sm dark:text-white/90">
                        {cat.titleEn} / {cat.titleAm}
                      </span>
                    </TableCell>
                    <TableCell className="px-4 py-3 text-start">
                      {cat.image ? (
                        <img src={cat.image} alt={cat.titleEn} className="w-12 h-8 object-cover rounded" />
                      ) : (
